...
class OrchestratorAgent:
        ...
        state.update(
            {
                ...
                "agent_statuses": {
                    ...
                    "documentation": AgentStatus.PENDING.value,
                },
    ...
    def log_workflow_event(
        self, state: Dict[str, Any], event: str, agent: str, details: str
    ):
    ...
    def log_inter_agent_communication(
        ...
        logger.info(
            f"[{state.get('claim_id')}] Communication: {from_agent} -> {to_agent}: {message}"
        )

    def handle_agent_error(
        self, state: Dict[str, Any], agent_name: str, error: Exception
    ) -> Dict[str, Any]:
        ...
        if retry_count < max_retries:
            ...
            logger.warning(
                f"Retrying agent {agent_name} (attempt {retry_count + 1}/{max_retries})"
            )
            return state
        else:
            ...
            logger.error(
                f"Workflow failed for claim {state.get('claim_id')}: {error_msg}"
            )
            return state
    ...
    def update_workflow_metrics(self, state: Dict[str, Any]):
        ...
        if processing_time > 0:
            ...
            self.workflow_metrics["average_processing_time"] = (
                total_time / self.workflow_metrics["total_claims_processed"]
            )
...
def intake_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        orchestrator.log_workflow_event(
            state, "agent_started", "intake", "Starting intake processing"
        )
        ...
        orchestrator.log_workflow_event(
            ...
            "agent_completed",
            "intake",
            f"Intake processing completed, next: {enhanced_result['next_agent']}",
        )
...
def risk_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        orchestrator.log_workflow_event(
            state, "agent_started", "risk_assessment", "Starting risk assessment"
        )
...
def routing_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        orchestrator.log_workflow_event(
            state, "agent_started", "routing", "Starting routing decision"
        )
        ...
        enhanced_state["total_processing_time"] = enhanced_state[
            "processing_end_time"
        ] - enhanced_state.get("processing_start_time", 0)
...
def determine_processing_mode(claim: Dict[str, Any]) -> str:
    ...
    if (
        claim.get("type") in ["auto_glass", "windshield"]
        and claim.get("amount", 0) < 1000
        and not claim.get("injuries_reported", False)
    ):
        return "fast_track"

    # Complex criteria: high amounts, injuries, or multiple parties
    if (
        claim.get("amount", 0) > 25000
        or claim.get("injuries_reported", False)
        or claim.get("other_party_involved", False)
        or claim.get("type") in ["liability", "medical_payment"]
    ):
        return "complex"
...
def final_routing_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        orchestrator.log_workflow_event(
            ...
            "agent_started",
            "final_routing",
            "Starting final routing with parallel optimization",
        )
        ...
        enhanced_state["agent_statuses"]["routing"] = AgentStatus.COMPLETED.value
        enhanced_state["workflow_status"] = (
        ...
        if (
            "parallel_results" in state
            and "preliminary_routing" in state["parallel_results"]
        ):
...
def fast_track_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        orchestrator.log_workflow_event(
            ...
            "fast_track_started",
            "orchestrator",
            "Starting fast-track processing",
        )
        ...
        fast_result["total_processing_time"] = fast_result[
            "processing_end_time"
        ] - fast_result.get("processing_start_time", 0)
...
def error_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    orchestrator.log_workflow_event(
        state, "error_handling", "orchestrator", f"Error node activated: {error_msg}"
    )
    ...
    enhanced_error_state["total_processing_time"] = enhanced_error_state[
        "processing_end_time"
    ] - enhanced_error_state.get("processing_start_time", 0)
...
def final_routing_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    routing_input = {
        "claim": state.get(
            "claim", state
        ),  # Use state["claim"] if available, otherwise fallback to state
        "risk_score": state.get("risk_score"),
        "risk_level": state.get("risk_level"),
        "risk_reasons": state.get("risk_reasons", []),
        "validation_errors": state.get("validation_errors", []),
...
def route_processing_mode(
    state: Dict[str, Any],
) -> Literal["fast_track", "parallel", "sequential"]:
    ...
    if mode == "fast_track":
        ...
        return (
            "sequential"  # Use sequential for complex claims that need careful analysis
        )
...
def documentation_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Documentation node for generating claim summaries and documentation using LLM"""
    try:
        orchestrator = OrchestratorAgent()

        # Update current agent and status
        state["current_agent"] = "documentation"
        state["agent_statuses"]["documentation"] = AgentStatus.PROCESSING.value

        orchestrator.log_workflow_event(
            state, "agent_started", "documentation", "Starting documentation generation"
        )

        # Prepare processing results for documentation agent
        processing_results = {
            "intake": {
                "is_valid": not bool(state.get("validation_errors")),
                "validation_errors": state.get("validation_errors", []),
            },
            "risk_assessment": {
                "risk_score": state.get("risk_score"),
                "risk_level": state.get("risk_level"),
                "fraud_indicators": state.get("risk_reasons", []),
            },
            "routing": {
                "priority": state.get("priority"),
                "adjuster_tier": state.get("adjuster_tier"),
                "processing_path": state.get("processing_path", "standard"),
            },
        }

        # Process with documentation agent
        doc_result = orchestrator.documentation_agent.process(state, processing_results)
        ...
        orchestrator.log_inter_agent_communication(
            ...
            "documentation",
            "orchestrator",
            f"Documentation generated: {len(doc_result.get('summary', ''))} char summary",
            {
                "status": doc_result.get("status"),
                "has_summary": bool(doc_result.get("summary")),
            },
        )

        # Update state with documentation results
        enhanced_state = {**state}
        enhanced_state["documentation_summary"] = doc_result.get("summary")
        enhanced_state["documentation_content"] = doc_result.get("documentation")
        enhanced_state["documentation_key_points"] = doc_result.get("key_points")
        enhanced_state["agent_statuses"]["documentation"] = AgentStatus.COMPLETED.value
        ...
        orchestrator.log_workflow_event(
            ...
            "workflow_completed",
            "orchestrator",
            f"Workflow with documentation completed in {enhanced_state['total_processing_time']:.2f}s",
        )

        return enhanced_state

    except Exception as e:
        logger.error(f"Error in documentation node: {str(e)}")
        return orchestrator.handle_agent_error(state, "documentation", str(e))
...
# Fast-track goes to documentation then end
graph.add_edge("fast_track", "documentation")

# Parallel processing goes to final routing then documentation
graph.add_edge("parallel_processing", "final_routing")
graph.add_edge("final_routing", "documentation")

# Sequential processing (for complex claims) goes to documentation
graph.add_edge("risk", "routing")
graph.add_edge("routing", "documentation")

# Documentation is the final step before END
